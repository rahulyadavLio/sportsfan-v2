...
export default function VisualComparison({ event, category }: VisualComparisonProps) {
  ...
  const maxValue = Math.max(...records.map(r => r.numericValue));
  const minValue = Math.min(...records.map(r => r.numericValue));
            ...
            <div key={record.type} role="listitem">
              <div className="flex items-center justify-between mb-2">
                <span className="text-sm text-[#99a1af] font-medium flex items-center gap-2">
                  <span className="w-3 h-3 rounded-full" style={{ backgroundColor: record.color }} aria-hidden="true"></span>
                  {record.type}
                </span>
